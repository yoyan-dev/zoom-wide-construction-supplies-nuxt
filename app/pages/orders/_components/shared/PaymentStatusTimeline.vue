...
import {
  buildPaymentTimelineEntries,
  type PaymentTimelineState,
} from "./payment-status";
...
const timelineEntries = computed(() => buildPaymentTimelineEntries(props.payment));

const markerClass = (state: PaymentTimelineState) => {
  switch (state) {
    case "complete":
...
const titleClass = (state: PaymentTimelineState) => {
  switch (state) {
    case "error":
